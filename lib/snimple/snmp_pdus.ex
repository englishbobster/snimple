defmodule Snimple.SnmpPdus do
 ...
 ## Example
  ...
  """
    ...
    def snmp_error(status) when is_atom(status) do
        error_status[status]
    end
    def snmp_error(status) when is_integer(status) do
    ...
    def encode_pdu(vblist, requid, :snmpget) do
        _encode_pdu(vblist, requid, snmp_error(:noError), 0, :snmpget)
    end

    def encode_pdu(vblist, requid, :snmpgetnext) do
        _encode_pdu(vblist, requid, snmp_error(:noError), 0, :snmpgetnext)
    end

    def encode_pdu(vblist, requid, :snmpset) do
        _encode_pdu(vblist, requid, snmp_error(:noError), 0, :snmpset)
    end

    def encode_pdu(vblist, requid, :snmptrap) do
        _encode_pdu(vblist, requid, snmp_error(:noError), 0, :snmptrap)
    end

    def encode_pdu(vblist, requid, :snmpinform) do
        _encode_pdu(vblist, requid, snmp_error(:noError), 0, :snmpinform)
    end
