defmodule BERTest do
	...
	test "should be able to encode assorted OIDs according to BER" do
		...
		assert ber(:oid, oid_1) == << 6, 15, 43, 6, 1, 4, 1, 196, 4, 2, 1, 2, 2, 1, 1, 3, 16 >>
		assert ber(:oid, oid_2) == << 6, 16, 43, 6, 1, 4, 1, 196, 4, 2, 4, 2, 2, 1, 1, 72, 141, 3 >>
		assert ber(:oid, oid_3) == << 6, 16, 43, 6, 1, 4, 1, 196, 4, 2, 4, 2, 0, 1, 1, 72, 141, 3 >>
		assert ber(:oid, oid_4) == <<6, 13, 43, 6, 1, 4, 1, 129, 155, 25, 1, 2, 1, 6, 0>>
		assert ber(:oid, oid_5) == <<6, 13, 43, 6, 1, 4, 1, 129, 155, 25, 1, 2, 1, 6, 0>>
		assert ber(:oid, oid_6) == <<6, 16, 43, 0, 1, 4, 1, 148, 120, 1, 2, 7, 3, 2, 129, 155, 25, 0 >>
	end
